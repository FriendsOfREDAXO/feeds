...
// Ensure all table structures for feeds addon
rex_sql_table::get(rex::getTable('feeds_stream'))
    ...
    ->ensureColumn(new rex_sql_column('whitelist', 'text'))
    ->ensureColumn(new rex_sql_column('blacklist', 'text'))
...
rex_sql_table::get(rex::getTable('feeds_item'))
    ...
    ->ensureColumn(new rex_sql_column('media', 'longblob'))
    ->ensureColumn(new rex_sql_column('mediasource', 'text'))
    ->ensureColumn(new rex_sql_column('media_filename', 'text'))
    ->ensureColumn(new rex_sql_column('raw', 'longblob'))
    ->ensureColumn(new rex_sql_column('status', 'tinyint(4)', false, '1'))
    ...
    ->ensureIndex(new rex_sql_index('stream_status_date', ['stream_id', 'status', 'date']))
